...
This databse for this project is generated by Python. 
...
To install Python in windows, simply download the release from [here](https://www.python.org/downloads/windows/) and follow the install instructions.\
</details>
...
Python is usually pre-installed on Linux. You can verify this via `python3 --version`. If you need to install Python on Linux, use `sudo apt install python3`.

### Python Virtual Environment (Recommended)
To avoid dependency conflicts, it is recommended to use a Python virtual environment. On Linux you will need to install the `venv` package via: `sudo apt install python3-venv'.
...
To install node in Windows using the [Installer](https://nodejs.org/en/download).  To install on a Mac, use the [Installer](https://nodejs.org/en/download) or `brew install node`.  For Linux follow the [installation for node using the node version manager](https://www.digitalocean.com/community/tutorials/how-to-install-node-js-on-ubuntu-22-04#option-3-installing-node-using-the-node-version-manager). For the version you can use the latest (`nvm install --lts`).
...
To install make in mac, do `brew install make` if you have [Homebrew](https://brew.sh/) installed. It is also available in other package managers. For Linux, it is recommended to acquire `make` through the development tools package: `sudo apt update` followed by `sudo apt install build-essential`.\
