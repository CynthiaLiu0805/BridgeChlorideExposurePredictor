...
\begin{description}
\item[\refstepcounter{ucnum} \uctheucnum \label{ucDevInput}:] The traffic data model and climate data model are inputted from developer and will be hidden to user.
...
\multirow{3}{0.3\textwidth}{Behaviour-Hiding Module} & Input Parameter Module\\
& Control Module\\
& Output Visualization Module\\

\midrule

\multirow{3}{0.3\textwidth}{Software Decision Module} & Data Searching Module\\
...
\begin{description}
\item[Secrets:] The format and structure of the input data. The kind of input data that is invalid.
\item[Services:] Get input from the user, verify if the input is within the physic and software constraint, throw warning message if it is not. Converts the input data into the data structure used by the Data Searching module.
...
\begin{description}
\item[Secrets:] None, the main purpose of this module is for usability that the result could be shown to user straightforwardly. 
\item[Services:] Provide the visualization of the resulting chloride exposure trend, using line graph or histogram. Allow user interaction with the visualization.
\item[Implemented By:] BC
...
\begin{description}
\item[Secrets:] Hide the irrelevant values in database.
...
\begin{description}
\item[Secrets:] Detail caculation formula for chloride exposure trend.
...
\midrule
R1 & \mref{mInput}, \mref{mControl}\\
R2 & \mref{mSearch}, \mref{mCalculation}, \mref{mControl}, \mref{mOutput}\\
R3 & \mref{mCalculation}\\
R4 & \mref{mCalculation}, \mref{mOutput}\\
\bottomrule
\end{tabular}
