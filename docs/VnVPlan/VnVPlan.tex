...
{ %
    \begin{tabular}{ lccccc }
    \hline
      \multicolumn{3}{c|}{Input (\si[per-mode=symbol] {\degree}) }                            & \multicolumn{2}{c}{Output} \\ 
    
    \hline
    %TODO: find model data
...
\subsubsection{Intermediate Tests - test\_model\_calculation}
...
\hline        
       Test case & Input & Output  \\ \hline
    test-deicing-salts-quantity & $M_{total}=$, $t_1=$ d-by-one-truck &  $M_{app}$        \\ \hline
    test-melted-water-thickness & &          \\ \hline
    test-water-sprayed-and-splashed & &          \\ \hline
    test-chloride-ions-sprayed-and-splashed-by-one-truck & &          \\ \hline
    test-chloride-ions-sprayed-and-splashed-by-all-vehicles& &          \\ \hline
    test-deicing-salts-deposition-on-the-surface-of-the-bridge-substructure& &          \\ \hline
\end{tabular} %
}
...
Input: The input columns in Table \ref{Table:calculation} 
					
Output: The output columns in Table \ref{Table:calculation} 

Test Case Derivation: The expected value in the output column is determined using the equation provided by (TODO: add reference)\ref{}. At each step, the test case verifies if the calculation for the corresponding step is accurate. If it is, the test proceeds to the next step. The input will be the random values selected from the models.
...
\subsubsection{Output Tests - test\_output}
...
\subsubsection{NFR: Maintainability}
